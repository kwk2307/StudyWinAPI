...
struct Math
{
	...
	__forceinline static constexpr int TruncToInt(float InFloat)
	{
		return (int)InFloat;
	}

	__forceinline static constexpr int RountToInt(float InFloat)
	{
		return TruncToInt(roundf(InFloat));
	}

	__forceinline static constexpr int FloorToInt(float InFloat)
	{
		return TruncToInt(floorf(InFloat));
	}

	__forceinline static int CeilToInt(float InFloat)
	{
		return TruncToInt(ceilf(InFloat));
	}

	__forceinline static constexpr bool EqualsInTolerance(float InFloat1, float InFloat2, float InTolerance = SMALL_NUMBER)
	{
		return Math::Abs(InFloat1 - InFloat2) <= InTolerance;
	}

	template< class T>
	__forceinline static constexpr T Lerp(const T& InSrc, const T& InDest, float InAlpha)
	{
		return (T)(InSrc + InAlpha * (InDest - InSrc));
	}

	template<class T>
	__forceinline static constexpr T Square(const T InNum)
	{
		return InNum * InNum;
	}

	__forceinline static constexpr float Deg2Rad(float InDegree)
	{
		return InDegree * PI / 180.f;
	}

	__forceinline static constexpr float Rad2Deg(float InRadian)
	{
	...
	template< class T >
	__forceinline static T constexpr Max3(const T A, const T B, const T C)
	{
		return Max(Max(A, B), C);
	}

	template< class T >
	__forceinline static T constexpr Min3(const T A, const T B, const T C)
	{
	...
	template<class T>
	__forceinline static constexpr T Clamp(const T X, const T Min, const T Max)
	{
		return X < Min ? Min : X < Max ? X : Max;
	}

	// 언리얼 엔진 코드에서 가져옴. 지정된 각도에 대한 두 삼각함수를 함께 가져오는 함수. 
	static __forceinline constexpr void GetSinCosRad(float& OutSin, float& OutCos, float InRadian)
	{
	...
	static __forceinline constexpr void GetSinCos(float& OutSin, float& OutCos, float InDegree)
	{
		if (InDegree == 0.f)
		{
			OutSin = 0.f;
	...
	__forceinline static float FMod(float X, float Y)
	{
	...
	__forceinline static float InvSqrt(float InFloat)
	{
